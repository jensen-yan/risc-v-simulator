...
public:
    ...
    struct StoreHazardInfo {
        StoreHazardKind kind = StoreHazardKind::None;
...
public:
    ...
    StoreHazardInfo get_earlier_store_hazard_info(uint64_t current_instruction_id,
                                                  uint64_t load_address,
                                                  uint8_t load_size) const;
    std::vector<DynamicInstPtr> get_earlier_address_unknown_stores(uint64_t current_instruction_id) const;
