...
void FetchStage::execute(CPUState& state) {
    ...
    state.perf_counters.increment(PerfCounterId::FETCH_SLOTS, OOOPipelineConfig::FETCH_WIDTH);
    ...
    if (state.fetch_buffer.size() >= MAX_FETCH_BUFFER_SIZE) {
        ...
        uint64_t next_fetch_pc = state.pc;
        size_t fetched_this_cycle = 0;

        for (size_t slot = 0;
             slot < OOOPipelineConfig::FETCH_WIDTH && state.fetch_buffer.size() < MAX_FETCH_BUFFER_SIZE;
             ++slot) {
            try {
