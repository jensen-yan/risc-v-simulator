...
// Store Forwarding Buffer 表项，用于 Store-to-Load Forwarding
struct StoreForwardingBufferEntry {
    bool valid;                     // 表项是否有效
    ...
    StoreForwardingBufferEntry() : valid(false), instruction(nullptr), address(0), value(0), size(0) {}
};

// Store Forwarding Buffer，用于实现 Store-to-Load Forwarding
class StoreForwardingBuffer {
public:
    // 需覆盖全部在飞store，避免提早发布后因环形覆盖丢失live entry。
    static const int MAX_ENTRIES = static_cast<int>(OOOPipelineConfig::STORE_FORWARDING_BUFFER_ENTRIES);
    ...
    // 调试：打印Store Forwarding Buffer状态
    void dump() const;
    size_t get_occupied_entry_count() const;
    size_t get_capacity() const { return MAX_ENTRIES; }

private:
    std::array<StoreForwardingBufferEntry, MAX_ENTRIES> entries;
...
private:
    ...
    bool can_extract_load_data(const StoreForwardingBufferEntry& store_entry, uint64_t load_addr, uint8_t load_size) const;
    
    // 从Store Forwarding Buffer条目中提取Load需要的数据
    uint64_t extract_load_data(const StoreForwardingBufferEntry& store_entry, uint64_t load_addr, uint8_t load_size) const;
};
