...
OOO 路径通过 `MemoryTimingBackend` 表达 core 外部的内存时序。L1I/L1D miss 会先访问一个 unified shared L2；L2 miss 再进入固定延迟或 DRAMSim3 后端。功能数据仍从 `Memory` 取，L2 目前只建模驻留与时序。

```mermaid
flowchart LR
  OOO["OutOfOrderCPU"] --> L1I["L1I NonBlockingCache"]
  OOO --> L1D["L1D NonBlockingCache"]
  L1I --> L2["Shared L2 NonBlockingCache"]
  L1D --> L2
  L2 --> Timing["MemoryTimingBackend"]
...
当前边界是 `L1 -> L2 -> memory timing backend -> Memory`。如果目标继续逼近 gem5/SPEC06 分数，下一层再考虑 L3 和更完整的 cache writeback/coherence；不要把 L3 或 DDR 行为混进 `Memory`。
