...
- [x] 2026-05-19 继续深化 `CommitStage`：抽 `CommitRegisterEffects`，集中整数/浮点架构寄存器、fflags、rename commit/update 规则，让 CommitStage 主循环只处理结果失败转 halt；`cmake --build build -j`、聚焦 CommitRegisterEffects/CommitStage/OOO 测试、全量 `ctest` 330/330 通过。
- [x] 2026-05-19 继续深化 `CommitStage`：抽 `CommitControlFlowEffects`，集中 BRANCH/JAL/JALR 的 predictor/profile/counter 更新与 redirect flush 判定，让 CommitStage 只消费 flush decision；`cmake --build build -j`、聚焦 CommitControlFlowEffects/CommitStage/BranchPredictor/OOO 测试、全量 `ctest` 335/335 通过。
- [x] 2026-05-19 继续深化 `CommitStage`：抽 `CommitSystemEffects`，集中 CSR / ECALL / EBREAK / MRET / FENCE.I 的串行化退休效果、trap 入口和 full-pipeline recovery，CommitStage 只消费 stop/flush metadata；`cmake --build build -j`、聚焦 CommitSystemEffects/CommitStage/OOO/Simulator 测试、全量 `ctest` 339/339 通过。
- [x] 2026-05-19 继续深化 `ExecuteStage`：抽 `ExecuteControlRecovery`，集中执行阶段控制流早恢复的 checkpoint restore / younger cleanup / RAS repair / recovery counters，让 ExecuteStage completion 只触发早恢复尝试；`cmake --build build -j`、聚焦 ExecuteControlRecovery/ExecuteStage/OOO/OooRecovery 测试、全量 `ctest` 342/342 通过。
- [x] 2026-05-19 继续深化 `ExecuteStage`：抽 `ExecuteDCacheAccess`，集中 D$ request start / blocked / latency / counter 规则，为后续 load/store execution 拆分建立更窄的 cache timing Module；`cmake --build build -j`、聚焦 ExecuteDCacheAccess/ExecuteStage/OutOfOrderCache/OOO 测试、全量 `ctest` 346/346 通过。
- [x] 2026-05-19 继续深化 `ExecuteStage`：抽 `ExecuteLoadValue`，集中 integer sign/zero extension 与 FLW NaN-boxing，让 store-forwarding 与 memory-load 两条路径共享同一条 load value formatting 规则；`cmake --build build -j`、聚焦 ExecuteLoadValue/ExecuteStage/OutOfOrderCache/OOO 测试、全量 `ctest` 351/351 通过。
- [x] 2026-05-19 继续深化 `ExecuteStage`：抽 `ExecuteHostCommAccess`，集中 tohost/fromhost MMIO range detection 与 ROB-head serialization 规则，让 load/store completion 只消费一个 must-serialize decision；`cmake --build build -j`、聚焦 ExecuteHostCommAccess/ExecuteStage/OutOfOrderCache/OOO 测试、全量 `ctest` 356/356 通过。
- [x] 2026-05-19 继续深化 `ExecuteStage`：抽 `ExecuteMemoryInflight`，集中已发出 D$ miss 的 inflight queue 移入、wait-cycle advancement、完成回调和 store violation recovery 调用；`cmake --build build -j`、聚焦 ExecuteMemoryInflight/ExecuteStage/OutOfOrderCache/OOO 测试、全量 `ctest` 362/362 通过。
- [x] 2026-05-19 继续深化 `ExecuteStage`：抽 `ExecuteLoadHazard`，集中 load completion 前 ROB store hazard 的 replay/continue decision、replay counter 与 caused-by counter 更新；`cmake --build build -j`、聚焦 ExecuteLoadHazard/ExecuteStage/OutOfOrderCache/OOO 测试、全量 `ctest` 367/367 通过。
- [x] 2026-05-19 继续深化 `ExecuteStage`：抽 `ExecuteLoadAccess`，集中 store-to-load forwarding、memory/cache read、partial merge、cache wait 与 exception result，让 ExecuteStage 只消费 load access result；`cmake --build build -j`、聚焦 ExecuteLoadAccess/ExecuteStage/OutOfOrderCache/OOO 测试、全量 `ctest` 371/371 通过。
- [x] 2026-05-19 继续深化 `ExecuteStage`：抽 `ExecuteStoreAccess`，集中 store host-comm serialization、D$ write timing、inflight movement 和 memory-order violation recovery trigger，让 ExecuteStage 只消费 store access result；`cmake --build build -j`、聚焦 ExecuteStoreAccess/ExecuteStage/OutOfOrderCache/OOO 测试、全量 `ctest` 375/375 通过。
- [x] 2026-05-19 继续深化 `ExecuteStage`：抽 `ExecuteLoadCompletion`，集中 ready load 的 host-comm replay、ROB hazard replay、load access result handling、cache/inflight follow-up 与 replay bucket，让 ExecuteStage 只消费 load completion result；`cmake --build build -j`、聚焦 ExecuteLoadCompletion/ExecuteLoadAccess/ExecuteLoadHazard/ExecuteStage/OutOfOrderCache/OOO 测试、全量 `ctest` 379/379 通过。
- [x] 2026-05-19 架构复盘后停止机械拆分：更新 `ARCHITECTURE.md` 的 OOO Execute/Commit 模块地图、性能探索入口索引、Recovery 所有权与拆分停止线，降低后续性能实验的导航成本。
