...
保持KISS 原则和DRY 原则！
对于复杂任务可以调用subagents, 所有subagent 默认使用gpt 5.4 high 模型！
...
## 文档维护约定
- 架构说明只保留一份，统一维护在仓库根目录 `ARCHITECTURE.md`。
- 不要在 `AGENTS.md`、`README.md`、`tasks/` 中复制大段架构描述，避免多处漂移。
- 目录职责、主数据流、核心分层发生变化时，提交代码时顺手更新 `ARCHITECTURE.md`。
